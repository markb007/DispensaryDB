$(document).ready($(function() {
    ...
    $('tr').click(function () {
        ...
        //  change href
        var myaction = $("a[id='actionSupplierUpdate']").attr('href');
        ...
        $("#actionSupplierUpdate").attr("href", result);

        var myaction = $("a[id='actionSupplierProducts']").attr('href');
        //  get second occurence of '/' from href and extract up to that then add on value string
        var tokens2 = myaction.split(delimiter).slice(0, 4);
        var fronthref = tokens2.join(delimiter) + delimiter;
        // now replace to the end
        var result = fronthref + myvalue + delimiter;
        $("#actionSupplierProducts").attr("href", result);
